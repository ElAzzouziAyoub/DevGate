<template>
    <div class="form-container">
      ...
      <form @submit.prevent="addDiscussion" class="discussion-form">
        ...
        <p class="error-message" v-show="submission_err !== ''">
          {{ submission_err }}
        </p>
        <button type="submit" class="submit-button">Add Discussion</button>
      </form>
    </div>
  </template>
  ...
  import { addNewDiscussion, addNewActivity } from '@/composables/useFirestore';
  ...
  const newDiscussionID = ref(null);
  ...
  async function addDiscussion(event) {
      event.preventDefault();
      if (title.value === "") {
          submission_err.value = "Empty discussion title.";
          return;
      }
      if (Categories.value.length === 0) {
          submission_err.value = "No categories defined.";
          return;
      }
      if (content.value === "") {
          submission_err.value = "Empty discussion content.";
          return;
      }
      submission_err.value = "";

      showPopUp(2, "Voulez-vous soummettre la discussion?", 31);
  }
...
async function ContinueOp(confEvent) {
    if(confEvent == 31) {
      ...
      let response = await addNewDiscussion(discussion);
      if(response.status) {
        ...
        if(activityRes.status) {
          newDiscussionID.value = response.id;
          showPopUp(1, "Discussion Ajoutée.", 32);
        } else {
          showPopUp(3, activityRes.error, 33);
        }
      } else {
          showPopUp(3, response.error, 33);
      }
    } else if(confEvent == 32) {
        router.push('/Discussion/' + newDiscussionID.value);
        hidePopUp();
    } else if(confEvent == 33) {
        hidePopUp();
    }
}
  </script>
